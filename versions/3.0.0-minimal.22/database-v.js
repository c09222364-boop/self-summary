...
#${ROOT_ID} .main{min-width:0;min-height:0;display:flex;flex-direction:column;overflow:hidden}.tabs{display:flex;gap:0;padding:0 13px;background:var(--paper);border-bottom:1px solid var(--line);flex:none;overflow-x:auto;white-space:nowrap;scrollbar-width:thin;-webkit-overflow-scrolling:touch}.tab{border:0;border-bottom:3px solid transparent;background:transparent;color:var(--muted);padding:0 21px;height:45px;cursor:pointer;font-size:13px;flex:0 0 auto}.tab:hover{color:var(--accent);background:var(--accent-soft)}.tab.active{color:var(--accent);font-weight:700;border-bottom-color:var(--accent)}.content{padding:13px 15px;overflow:auto;flex:1;min-height:0;background:var(--canvas);-webkit-overflow-scrolling:touch}.content.data-content{display:flex;flex-direction:column;overflow:hidden}.data-content .toolbar{flex:none}.data-content .table-wrap{flex:1;min-height:0;overflow:auto;-webkit-overflow-scrolling:touch}.toolbar{display:flex;align-items:center;gap:7px;flex-wrap:wrap;margin-bottom:9px}.toolbar .grow,.foot .grow{flex:1}.toolbar strong{font-size:15px}.btn{border:1px solid var(--accent);background:var(--accent);color:#fff;border-radius:4px;padding:7px 11px;min-height:34px;cursor:pointer;font-size:13px;transition:filter .12s,background .12s,border-color .12s}.btn:hover{filter:brightness(.96)}.btn.secondary{border-color:var(--line);background:var(--paper);color:var(--ink)}.btn.secondary:hover{border-color:var(--accent);color:var(--accent);background:var(--accent-soft)}.btn.warn{border-color:var(--danger);background:transparent;color:var(--danger)}
...
#${ROOT_ID} .table-wrap{overflow:auto;border:1px solid var(--line);border-radius:5px;background:var(--paper-warm);scrollbar-width:thin}.data-table{border-collapse:collapse;width:100%;min-width:520px}.data-table th,.data-table td{border-bottom:1px solid var(--line-soft);padding:7px 9px;text-align:left;vertical-align:top}.data-table th{background:var(--paper2);font-size:12px;color:var(--muted);position:sticky;top:0;z-index:1}.data-table td[contenteditable=true]{min-width:84px;max-width:300px;outline:none;white-space:pre-wrap;overflow-wrap:anywhere}.data-table td[contenteditable=true]:focus{background:var(--accent-soft);box-shadow:inset 0 0 0 1px var(--accent)}.data-table th:last-child,.data-table td:last-child{width:62px;white-space:nowrap}.data-table td:last-child .minibtn{padding:5px 7px;min-height:30px}.row-id{color:var(--muted);font-family:ui-monospace,Consolas,monospace;font-size:12px;white-space:nowrap}.empty{padding:32px;text-align:center;color:var(--muted)}
...
#${ROOT_ID} #acu-sheet-list,#${ROOT_ID} .content,#${ROOT_ID} .data-content .table-wrap{min-width:0}
...
`;
    ...
    function renderData(element, sheet) {
        ...
        element.innerHTML = `<div class="toolbar"><strong>${esc(sheet.name)}</strong><span class="hint">${rows.length} 行 · 第 ${app.page + 1}/${layout.pageCount} 页 · 当前 ${orderLabel}</span><button class="minibtn" data-act="prev-page" ${app.page <= 0 ? 'disabled' : ''}>上一页</button><button class="minibtn" data-act="next-page" ${app.page >= layout.pageCount - 1 ? 'disabled' : ''}>下一页</button><button class="minibtn" data-act="toggle-row-order">${app.reverseRows ? '切换正序' : '切换倒序'}</button><span class="grow"></span><button class="btn secondary" data-act="add-row">＋ 添加行</button></div>
          <div class="table-wrap"><table class="data-table"><thead><tr><th>#</th>${header.slice(1).map(value => `<th>${esc(value)}</th>`).join('')}<th>操作</th></tr></thead><tbody>
          ${layout.items.map(item => `<tr><td class="row-id">${esc(item.row?.[0])}</td>${header.slice(1).map((_, columnIndex) => `<td contenteditable="true" data-cell-row="${item.physicalIndex + 1}" data-cell-col="${columnIndex + 1}">${esc(item.row?.[columnIndex + 1])}</td>`).join('')}<td><button class="minibtn danger" data-act="delete-row" data-row="${item.physicalIndex + 1}">删除</button></td></tr>`).join('')}
          </tbody></table>${rows.length ? '' : '<div class="empty">还没有数据行，点击“添加行”。</div>'}</div>`;
    }
    ...
    function renderStatus(element) {
        ...
        element.innerHTML = `<section class="card"><h2>当前聊天状态</h2><div class="status-list">
          ...
          </div><p class="hint" style="margin-top:10px">这里的 checkpoint 就是聊天记录里的一份完整表格快照。编辑器只在你点击保存时写入最近 AI 楼层，不监听生成，也不会自动填表。</p></section>`;
    }
